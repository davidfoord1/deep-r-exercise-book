...
## Pockhammer and the gsl package
...
> ***diff** can be considered an inverse to **cumsum**. It computes the iterated difference: subtracts the first two elements, then the second from the third one, the third from the fourth, and so on. In other words, `diff(x)` gives* $y$ such that $y~i~ =x~i+1~ − x~i~$[^numeric-4].

[^numeric-4]: Not sure how to format correctly, there's supposed to be subscripts here y~i~ =x~i+1~ −x~i~, but it's not working in the mathematical notation format.
