...
## Exponentials and logarithms are opposites.
...
I'll just include the very nice example provided.

```{r x-example}
x <- seq(0, 10, length.out=1001)
```

Noting that you can design a grid layout for graphics with `mfrow` and `mfcol`.
...
I understand it as natural log (i.e. `log()` with its default `base = exp(1)` is the opposite function to computing the exponential.
